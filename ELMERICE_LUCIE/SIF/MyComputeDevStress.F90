...
!------------------------------------------------------------------------------
  ...
  ELSE
     ...
     IF ( ASSOCIATED( FlowVariable ) ) THEN
        ...
        IF (.NOT.ASSOCIATED(FlowValues)) THEN
          CALL FATAL('ComputeDevStress', 'FlowValues is NOT associated! SSAVelocity not found in memory.')
        ELSE
          WRITE(*,*) 'First few FlowValues:', FlowValues(1:min(4,SIZE(FlowValues)))
          ! Vérifie si tous les FlowValues sont 0
          IF (MAXVAL(ABS(FlowValues)) < 1.0e-10_dp) THEN
            WRITE(*,*) 'Warning: FlowValues are near zero everywhere.'
          END IF
        END IF
  ...
  SELECT CASE(FlowSolverName)
     ...
     CASE('Porous', 'porous')
        iSolverName = 2
     CASE('SSAVelocity', 'ssavelocity')
        iSolverName = 3
     CASE DEFAULT
        CALL FATAL('ComputeDevStress', '>Flow Solver Name< must be either "Flow Solution", "SSAVelocity" or "Porous"')
  END SELECT 
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
!   Get Rho_i and g and thickness when using SSA
!------------------------------------------------------------------------------
  IF (iSolverName == 3) THEN
     rho_i = GetConstReal(Model % Constants , 'Ice Density', GotIt)
     g = GetConstReal(Model % Constants, 'Gravitation', GotIt)
     IF (.NOT.GotIt) THEN
        CALL WARN('ComputeDevStress', 'No Keyword >rho_i< or >g< defined despite using SSA')
     END IF   
  END IF
...
!------------------------------------------------------------------------------           
           ...
           !!!!!!! When the Flow solver is SSA
           ELSE IF (iSolverName == 3) THEN
             !! Check for the presence of keywords related to the new vectorized Stokes solver
                 !TempName = GetString( Material, 'Temperature Field Variable', GotIt_TFV)
                 !IF (.NOT.(GotIt_CT .OR. GotIt_TFV)) THEN
                  !  CALL FATAL('ComputeDevStress', '>Constant Temperature< or >Temperature Field Variable< &
              ...
              LocalH(1:n) = ListGetReal( BodyForce , 'Thickness', n, NodeIndexes, GotIt)
              IF(.NOT.GotIt) CALL FATAL('ComputeDevStress','Variable >h< for thickness not found.')
           END IF
...
! Do we want to return Cauchy or deviatoric stresses ? Deviatoric by default
	   Cauchy = ListGetLogical( Material , 'Cauchy', Gotit )
           ...
           LocalVelo = 0.0_dp
           IF (iSolverName == 3) THEN
             ! remplissage local velo     
             DO i=1, 2
               LocalVelo(i,1:n) = FlowValues((dim)*(FlowPerm(NodeIndexes(1:n))-1) + i) ! SSA does not export p, 
             END DO
             ! preparation du calcul de w, définition dBasis
             ! redeclarer dbsis correctement...  
             stat = ElementInfo(CurrentElement, ElementNodes, u,v,w, detJ,Basis,dBasisdx, ddBasisddx, .FALSE., .FALSE.)            
             
             !IF (stat /= 0) THEN
             !  CALL FATAL("ComputeDevStress", "ElementInfo failed with status ")
             !END IF

             !WRITE(*,*) 'dBasisdx(1,1:2) = ', dBasisdx(1,1), dBasisdx(1,2)
             LGrad = 0.0_dp
             LGrad(1:2,1:2) = MATMUL( LocalVelo(1:2,1:n), dBasisdx(1:n,1:2) )
             !WRITE(*,*) 'LGrad(1,1) = ', LGrad(1,1), '  LGrad(2,2) = ', LGrad(2,2)
             LocalVelo(3,1:n) = (- (LGrad(1,1) + LGrad(2,2))) * LocalH(1:n) ! mass conservation in SSA
             !DO i = 1, MIN(n,5) ! test print w
             !   WRITE(*,*) 'Node ', i, ': w = ', LocalVelo(3,i)
             !END DO
     
           ELSE
             !DO i=1, dim ! SSA is not Bulked
             !  LocalVelo(i,1:n) = FlowValues((dim+1)*(FlowPerm(NodeIndexes(1:n))-1) + i)
             !END DO
           END IF
           BodyForce => GetBodyForce()
           IF ( dim < 3 .AND. OutOfPlaneFlow .AND. iSolverName < 3) THEN
           ...
           IF (iSolverName == 3) THEN
              !LGrad = 0.0_dp
              !LGrad = MATMUL( LocalVelo(:,1:n), dBasisdx(1:n,:) )
              LocalP(1:n) = rho_i * g * LocalH(1:n) + 2 * LocalViscosity(1:n) * LGrad(3,3) 
              !DO i = 1, MIN(n,5) ! test print P
              !   WRITE(*,*) 'Node ', i, ': P = ', LocalP(i)
              !END DO     
           ELSE 
